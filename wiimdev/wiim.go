...
func (device *WiimDevice) commandOK(command string, name string) error {
	var result string
...
func (device *WiimDevice) commandBool(command string, on bool, name string) error {
	value := 0
	if on {
		value = 1
	}
	return device.commandOK(fmt.Sprintf("%s:%d", command, value), name)
}
...
func (device *WiimDevice) SetAudioOutput(output int) error {
	return device.commandOK(fmt.Sprintf("setAudioOutputHardwareMode:%d", output+1),
...
func (device *WiimDevice) SetBalance(level float64) error {
	return device.commandOK(fmt.Sprintf("setChannelBalance:%f", level), "setChannelBalance")
}
...
func (device *WiimDevice) SetFadeEffects(on bool) error {
	return device.commandBool("SetFadeFeature", on, "setChannelBalance")
}
...
func (device *WiimDevice) SetFixedVolume(on bool) error {
	return device.commandBool("setVolumeControl", on, "setVolumeControl")
}
